            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg shadow-indigo-100/20 dark:shadow-none sm:rounded-2xl border border-gray-100 dark:border-gray-700 p-8">
                ...
                <form method="POST" action="{{ route('staff.update', $staff->id) }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select id="role" name="role" required
                                class="block w-full rounded-xl border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900/50 text-gray-900 dark:text-gray-100 focus:ring-[#39D3C4] focus:border-[#39D3C4] shadow-sm sm:text-sm transition-colors py-2.5">
                                <option value="Clinic Owner" {{ $staff->hasRole('Clinic Owner') ? 'selected' : '' }}>Doctor / Clinic Owner (Full Access)</option>
                                <option value="Secretary" {{ $staff->hasRole('Secretary') ? 'selected' : '' }}>Secretary / Receptionist (Front Desk Access)</option>
                                <option value="Dentist" {{ $staff->hasRole('Dentist') ? 'selected' : '' }}>Dentist (Clinical Access)</option>
                                <option value="Assistant" {{ $staff->hasRole('Assistant') ? 'selected' : '' }}>Assistant (Limited Clinical Access)</option>
                            </select>
                    ...
                    <div class="mt-10 mb-8 border-b border-gray-200 dark:border-gray-700 pb-6">
                        ...
                        <p class="mt-1 text-sm text-gray-500">Update the password for this staff member (Optional).</p>
                    </div>
